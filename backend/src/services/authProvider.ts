...
export const authProviderService = {
   ...
   */
  verifyGoogleToken: async (idToken: string): Promise<VerifiedGoogleUser> => {
    if (!idToken || typeof idToken !== 'string') {
      throw new Error('Google ID token is required.');
    }

    const currentClientId = process.env.GOOGLE_CLIENT_ID;
    if (!currentClientId || currentClientId.includes('mock')) {
      throw new Error('Google OAuth 2.0 Web Client ID is not configured on the server. Please provide a valid Google Client ID.');
    }
   ...
   */
  sendSmsOtp: async (phone: string): Promise<{ success: boolean; message: string }> => {
    ...
    if (!accountSid || !authToken || !verifySid) {
      ...
      throw new Error(
        'SMS Gateway unconfigured. Please configure TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_VERIFY_SERVICE_SID in backend/.env to deliver real SMS OTP to mobile devices.'
      );
    }
   ...
   */
  verifySmsOtp: async (phone: string, code: string): Promise<boolean> => {
    ...
    if (!accountSid || !authToken || !verifySid) {
      throw new Error(
        'SMS Gateway unconfigured. Please configure TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_VERIFY_SERVICE_SID in backend/.env to verify real SMS OTP.'
      );
    }
