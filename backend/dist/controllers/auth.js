...
const registerSchema = zod_1.z.object({
    username: zod_1.z.string().min(3, 'Username must have at least 3 characters'),
    email: zod_1.z.string().email('Invalid email address'),
    phone: zod_1.z.string().regex(/^\+?91?[6-9]\d{9}$/, 'Please enter a valid 10-digit Indian phone number (+91)'),
    password: zod_1.z.string().min(8, 'Password must be at least 8 characters long'),
    location: zod_1.z.string().min(2, 'Please select your Tamil Nadu district / location'),
});
const loginSchema = zod_1.z.object({
    identifier: zod_1.z.string().min(1, 'Email or phone number is required'),
...
exports.authController = {
    ...
    register: async (req, res) => {
        try {
            ...
            await email_1.emailService.sendWelcomeEmail(newUser.email, newUser.username).catch(() => { });
    ...
    login: async (req, res) => {
        try {
            ...
            if (!isMatch) {
                await User_1.User.findByIdAndUpdate(user._id, { $inc: { failedLoginCount: 1, loginAttemptCount: 1 } });
    ...
    civicGoogleLogin: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
    ...
    civicSendOtp: async (req, res) => {
        try {
            ...
            const result = await authProvider_1.authProviderService.sendSmsOtp(cleanPhone);
            res.json({
                success: true,
    ...
    civicVerifyOtp: async (req, res) => {
        try {
            ...
            const cleanPhone = phone.startsWith('+91') ? phone : `+91${phone.replace(/\D/g, '').slice(-10)}`;
            await authProvider_1.authProviderService.verifySmsOtp(cleanPhone, otp);
    ...
    officerRequestAccess: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            ...
            const existingOfficer = await User_1.User.findOne({ email: normalizedEmail, role: 'OFFICER' });
            if (existingOfficer) {
            ...
            const existingReq = await OfficerRequest_1.OfficerRequest.findOne({ email: normalizedEmail, status: 'PENDING' });
            if (existingReq) {
                res.status(400).json({
                    success: false,
                    message: 'An access application for this official email is already pending Controller review.',
            ...
            const accountNumber = await (0, Counter_1.getNextAccountNumber)();
            const dummyHashedPassword = await bcryptjs_1.default.hash(`OfficerInit@${Date.now()}`, 10);
            const username = name.toLowerCase().replace(/[^a-z0-9]/g, '_').substring(0, 14) +
                '_' +
    ...
    officerGoogleLogin: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            const { credential, idToken } = req.body;
            const token = credential || idToken;
            ...
            const googleUser = await authProvider_1.authProviderService.verifyGoogleToken(token);
            const email = googleUser.email.toLowerCase().trim();
            const user = await User_1.User.findOne({ email, role: 'OFFICER' });
            if (!user) {
                res.status(403).json({
                    success: false,
    ...
    officerSetPassword: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
    ...
    employeeChangePassword: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            ...
            user.password = await bcryptjs_1.default.hash(newPassword, 10);
            await user.save();
            await Employee_1.Employee.findOneAndUpdate({ userId: user._id }, { mustChangePassword: false });
            res.json({
                success: true,
    ...
    deleteAccount: async (req, res) => {
        try {
            ...
            await Employee_1.Employee.findOneAndDelete({ userId: req.user.id });
            res.clearCookie('accessToken');
            res.clearCookie('refreshToken');
            res.json({
                success: true,
    ...
    getMe: async (req, res) => {
        ...
        if (req.user.role === 'EMPLOYEE') {
            employeeProfile = await Employee_1.Employee.findOne({ userId: req.user._id });
        }
        res.json({
            success: true,
            user: {
    ...
    updateMe: async (req, res) => {
        try {
            ...
            const updated = await User_1.User.findByIdAndUpdate(req.user.id, {
                ...(location ? { location } : {}),
                ...(avatarUrl ? { avatarUrl } : {}),
                ...(phone ? { phone } : {}),
                ...(name ? { name } : {}),
    ...
    officerCreateProfileChangeRequest: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            if (!req.user || req.user.role !== 'OFFICER') {
    ...
    officerGetProfileChangeRequests: async (_req, res) => {
        res.json({ success: true, requests: [] });
    },
    getGoogleConfig: async (_req, res) => {
    ...
    saveGoogleClientId: async (req, res) => {
