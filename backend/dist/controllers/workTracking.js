...
exports.workTrackingController = void 0;
 ...
 */
async function resolvePhotoUrl(req) {
    const file = req.file;
    if (file && file.buffer) {
        return (0, upload_1.uploadToCloudinary)(file.buffer);
    }
 ...
 */
async function findEmployeeForUser(req, res) {
 ...
 */
async function findAssignedComplaint(complaintId, employeeId, res) {
    const complaint = await Complaint_1.Complaint.findOne({ _id: complaintId, assignedEmployeeId: employeeId });
    if (!complaint) {
        res.status(403).json({
            success: false,
...
exports.workTrackingController = {
     ...
     */
    completeSiteVisit: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            const employee = await findEmployeeForUser(req, res);
            if (!employee)
    ...
    completeWork: async (req, res) => {
        try {
            ...
            if (citizen) {
                ...
                await email_1.emailService.sendComplaintStatusUpdate(citizen.email, complaint.complaintId, 'RESOLVED', description.trim()).catch(() => { });
            }
     ...
     */
    getComplaintTracking: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            const { id } = req.params;
            ...
            if (req.user?.role === 'CITIZEN') {
                const reportedId = complaint.reportedById?._id?.toString() || complaint.reportedById?.toString();
            ...
            if (req.user?.role === 'EMPLOYEE') {
                ...
                const assignedId = complaint.assignedEmployeeId?._id?.toString() || complaint.assignedEmployeeId?.toString();
            ...
            const [events, evidence] = await Promise.all([
                ComplaintEvent_1.ComplaintEvent.find({ complaintId: complaint._id }).sort({ timestamp: 1 }).lean(),
                ComplaintEvidence_1.ComplaintEvidence.find({ complaintId: complaint._id }).sort({ uploadedAt: 1 }).lean(),
            ]);
            const now = new Date();
            const complaintAge = now.getTime() - new Date(complaint.createdAt).getTime();
            let actualWorkDuration = null;
            if (complaint.workStartedAt) {
                if (complaint.completedAt) {
            ...
            const assignedEmp = complaint.assignedEmployeeId;
            res.json({
                success: true,
                complaint: {
     ...
     */
    getComplaintEvents: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            const events = await ComplaintEvent_1.ComplaintEvent.find({ complaintId: req.params.id })
                .sort({ timestamp: 1 })
                .lean();
            res.json({
                success: true,
     ...
     */
    getComplaintEvidence: async (req, res) => {
        try {
            await (0, db_1.connectDb)();
            const { id } = req.params;
            const complaint = await Complaint_1.Complaint.findById(id).lean();
            if (!complaint) {
                res.status(404).json({ success: false, message: 'Complaint not found.' });
                return;
            }
            ...
            const evidence = await ComplaintEvidence_1.ComplaintEvidence.find({ complaintId: id })
                .sort({ uploadedAt: 1 })
                .lean();
            res.json({
                success: true,
