...
async function runTestSuite() {
    ...
    try {
        ...
        const officerHashedPw = await bcryptjs_1.default.hash('OfficerSecret@123', 10);
        officerUser = await User_1.User.create({
        ...
        const empAccNum = await (0, Counter_1.getNextAccountNumber)();
        const empHashedPw = await bcryptjs_1.default.hash(initialPassword, 10);
        employeeUser = await User_1.User.create({
        ...
        employeeRecord = await Employee_1.Employee.create({
            employeeId: empId,
        ...
        const fetchedUser = await User_1.User.findById(employeeUser._id).select('+password');
        ...
        const isMatchEmail = await bcryptjs_1.default.compare(initialPassword, fetchedUser.password);
        if (!isMatchEmail)
            throw new Error('Password mismatch on email login!');
        ...
        const foundById = await Employee_1.Employee.findOne({ employeeId: employeeRecord.employeeId });
        if (!foundById)
            throw new Error('Employee not found by Employee ID!');
        ...
        const foundByPhone = await Employee_1.Employee.findOne({ phone: employeeRecord.phone });
        if (!foundByPhone)
            throw new Error('Employee not found by phone number!');
        ...
        const isWrongMatch = await bcryptjs_1.default.compare('WrongPassword@999', fetchedUser.password);
        if (isWrongMatch)
            throw new Error('Authentication permitted invalid password!');
        ...
        await User_1.User.findByIdAndUpdate(employeeUser._id, { accountStatus: 'DISABLED' });
        const disabledEmp = await Employee_1.Employee.findOne({ employeeId: employeeRecord.employeeId });
        ...
        await employeeRecord.save();
        await User_1.User.findByIdAndUpdate(employeeUser._id, { accountStatus: 'ACTIVE' });
        ...
        const newHashedPw = await bcryptjs_1.default.hash(updatedPassword, 10);
        employeeUser.password = newHashedPw;
        await employeeUser.save();
        employeeRecord.mustChangePassword = false;
        await employeeRecord.save();
        const verifyUpdatedUser = await User_1.User.findById(employeeUser._id).select('+password');
        const isNewPassValid = await bcryptjs_1.default.compare(updatedPassword, verifyUpdatedUser?.password || '');
        if (!isNewPassValid)
        ...
        assignedComplaint = await Complaint_1.Complaint.create({
        ...
        unassignedComplaint = await Complaint_1.Complaint.create({
            complaintId: `TN-CHN-2026-${Math.floor(10000 + Math.random() * 90000)}`,
        ...
        const employeeReports = await Complaint_1.Complaint.find({ assignedEmployeeId: employeeRecord._id });
        ...
        const verificationLog = await ReportVerification_1.ReportVerification.create({
        ...
        await User_1.User.findByIdAndUpdate(employeeUser._id, {
            presenceStatus: 'OFFLINE',
            isOnline: false,
            lastSeenAt: new Date(),
        });
        const userAfterLogout = await User_1.User.findById(employeeUser._id);
        const empAfterLogout = await Employee_1.Employee.findById(employeeRecord._id);
        ...
        const reloadedEmp = await Employee_1.Employee.findOne({ employeeId: employeeRecord.employeeId });
        const reloadedUser = await User_1.User.findById(reloadedEmp?.userId);
        ...
        // Clean up test data
        if (officerUser)
            await User_1.User.findByIdAndDelete(officerUser._id);
        if (employeeUser)
            await User_1.User.findByIdAndDelete(employeeUser._id);
        if (employeeRecord)
            await Employee_1.Employee.findByIdAndDelete(employeeRecord._id);
        if (assignedComplaint)
            await Complaint_1.Complaint.findByIdAndDelete(assignedComplaint._id);
        if (unassignedComplaint)
            await Complaint_1.Complaint.findByIdAndDelete(unassignedComplaint._id);
