...
export const userController = {
  ...
  getAll: async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const enrichedUsers = await Promise.all(
        usersList.map(async (u) => {
          const [reportsCount, assignedCount, resolvedCount] = await Promise.all([
            Complaint.countDocuments({ reportedById: u._id }),
            Complaint.countDocuments({ assignedToId: u._id }),
            Complaint.countDocuments({ assignedToId: u._id, status: 'RESOLVED' }),
          ]);

          const { password: _, ...userSafe } = u as any;
          return {
            ...userSafe,
            id: u._id.toString(),
            reportsCount,
      ...
      const [totalCitizens, totalOfficers, totalAdmins, totalEmployees, totalReports] =
        await Promise.all([
  ...
  getById: async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const [userComplaints, assignedComplaints] = await Promise.all([
        Complaint.find({ reportedById: id }).sort({ createdAt: -1 }).limit(50).lean(),
        Complaint.find({ assignedToId: id }).sort({ createdAt: -1 }).limit(50).lean(),
      ]);
  ...
  deleteUser: async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      await OfficerRequest.deleteMany({ applicantId: id });
  ...
  getSuspicious: async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const phoneClusters = await User.aggregate([
        { $match: { phone: { $ne: null } } },
  ...
  getOfficerAccessRequests: async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const requests = await OfficerRequest.find(filter).sort({ createdAt: -1 }).lean();
  ...
  approveOfficerAccessRequest: async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      let request = await OfficerRequest.findById(id);
      let user: any = null;

      if (request) {
        user = await User.findOne({ email: request.email.toLowerCase().trim() });
      } else {
        user = await User.findById(id);
        if (user) {
          request = await OfficerRequest.findOne({ email: user.email.toLowerCase().trim() });
        }
      }
      ...
      if (!user && request) {
        ...
        const username =
          request.name.toLowerCase().replace(/[^a-z0-9]/g, '_').substring(0, 14) +
          '_' +
      ...
      await OfficerRequest.updateMany(
  ...
  getProfileChangeRequests: async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
    res.json({ success: true, count: 0, requests: [] });
  },

  approveProfileChangeRequest: async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
    res.json({ success: true, message: 'Profile request approved.' });
  },

  rejectProfileChangeRequest: async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
