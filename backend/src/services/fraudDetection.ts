...
export interface FraudEvaluationResult {
  score: number;
  flags: { reason: string; score: number }[];
...
export interface UserContext {
  ...
  previousComplaintCoordinates: { latitude: number; longitude: number }[];
  accountsWithSamePhoneCount: number;
 ...
 */
export function evaluateComplaintFraud(
  complaint: ComplaintInput,
  user: UserContext
): FraudEvaluationResult {
  const flags: { reason: string; score: number }[] = [];
  let scoreGained = 0;

  // (1) Duplicate photos check: Check if any uploaded photo URL has been used in previous complaints
  const hasDuplicatePhoto = complaint.photoUrls.some((url) =>
    user.existingPhotoUrls.includes(url)
  );
  ...
  if (user.previousComplaintCoordinates.length > 0) {
    ...
    const distKm = calculateDistance(
      recentCoord.latitude,
      recentCoord.longitude,
      complaint.latitude,
      complaint.longitude
    );
