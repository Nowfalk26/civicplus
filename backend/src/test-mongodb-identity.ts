...
async function runTests() {
  ...
  const controller = await User.findOne({ email: 'nowfal@gmail.com' });
  ...
  await User.deleteMany({ email: { $in: ['test_citizen@example.com', 'test_officer@tn.gov.in', 'test_emp@civic.tn.gov.in'] } });
  await Employee.deleteMany({ email: 'test_emp@civic.tn.gov.in' });
  await Complaint.deleteMany({ location: 'Test Area Verification' });
  await OfficerRequest.deleteMany({ email: 'test_officer@tn.gov.in' });
  ...
  const initialAccNum = await getNextAccountNumber();
  ...
  const userAfterLogout = await User.findById(user1.id);
  ...
  if (!userLookup) throw new Error('Existing user not found by authProviderUserId!');
  if (userLookup.id !== user1.id) throw new Error('Duplicate user created instead of reusing existing account!');
  ...
  const totalWithSameGoogleId = await User.countDocuments({ authProviderUserId: stableGoogleId });
  ...
  const officerUser = await User.create({
    ...
    phone: '+919876543288',
    password: await bcrypt.hash('InitPass@123', 10),
    role: 'OFFICER',
  ...
  await OfficerRequest.findOneAndUpdate({ email: officerUser.email }, { status: 'APPROVED', reviewedAt: new Date(), reviewedBy: controller._id });
  ...
  const empUserAccNum = await getNextAccountNumber();
  ...
  const empUser = await User.create({
    ...
    phone: '+919876543277',
    password: await bcrypt.hash('TNStaff@1234', 10),
    role: 'EMPLOYEE',
  ...
  const employeeAssignedReports = await Complaint.find({ assignedEmployeeId: employee._id });
  ...
  const employeeCanSeeOther = await Complaint.findOne({ _id: otherComplaint._id, assignedEmployeeId: employee._id });
  ...
  await complaint.save();
  const citizenStillAlive = await User.findById(user1._id);
  ...
  await Complaint.deleteMany({ _id: { $in: [complaint._id, otherComplaint._id] } });
  await AssignmentHistory.deleteMany({ complaintId: complaint._id });
  await ReportVerification.deleteMany({ complaintId: complaint._id });
  await Employee.deleteMany({ _id: employee._id });
  await User.deleteMany({ _id: { $in: [user1._id, officerUser._id, empUser._id] } });
  await OfficerRequest.deleteMany({ applicantId: officerUser._id });
