...
export interface TimelineRecord {
  id: string;
  stage: 'SUBMITTED' | 'ACCEPTED' | 'ASSIGNED' | 'IN_PROGRESS' | 'RESOLVED' | 'REJECTED';
  timestamp: string;
  officerName?: string;
  notes?: string;
  complaintId: string;
...
export interface ComplaintRecord {
  id: string;
  complaintId: string;
  category: 'ROAD_DAMAGE' | 'STREET_LIGHT' | 'ELECTRICAL_WIRE' | 'GARBAGE_WASTE' | 'STORM_WATER_DRAIN' | 'PUBLIC_SPACE';
  description: string;
  location: string;
  latitude: number;
  longitude: number;
  status: 'SUBMITTED' | 'ACCEPTED' | 'ASSIGNED' | 'IN_PROGRESS' | 'RESOLVED' | 'REJECTED';
...
const CATEGORIES: ('ROAD_DAMAGE' | 'STREET_LIGHT' | 'ELECTRICAL_WIRE' | 'GARBAGE_WASTE' | 'STORM_WATER_DRAIN' | 'PUBLIC_SPACE')[] = [
  'ROAD_DAMAGE',
  'STREET_LIGHT',
  'ELECTRICAL_WIRE',
  'GARBAGE_WASTE',
...
const STATUSES: ('SUBMITTED' | 'ACCEPTED' | 'ASSIGNED' | 'IN_PROGRESS' | 'RESOLVED' | 'REJECTED')[] = [
  'SUBMITTED',
  'ACCEPTED',
  'ASSIGNED',
  'IN_PROGRESS',
...
const SAMPLE_PHOTO_PAIRS: Record<string, { before: string; after: string }> = {
  ROAD_DAMAGE: {
...
export function buildSeedData() {
  ...
  const controllerPasswordHash = bcrypt.hashSync('Admin@123', 8);
