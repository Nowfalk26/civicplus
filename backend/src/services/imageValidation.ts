 ...
 */
async function extractImageBuffer(photoInput: string): Promise<{ buffer: Buffer; mime: string; hash: string }> {
  let mime = 'image/jpeg';
  let buffer: Buffer;
  ...
  const hash = crypto.createHash('sha256').update(buffer.slice(0, 10000)).digest('hex');
 ...
 */
function analyzeVisualCharacteristics(
  buffer: Buffer,
  mime: string,
  photoInput: string,
  ...
  for (let i = 50; i < buffer.length - 4; i += sampleStep) {
    ...
    if (b > r * 1.25 && b > g * 1.1) blueRatioSum++;
  ...
  // 2. Road Damage:
  const isRoadVisual =
    (edgeDensity > 0.06 && blueRatio < 0.38) ||
  ...
  // 5. Garbage Waste:
  const isGarbageVisual =
    lowerInput.includes('garbage') ||
 ...
 */
export async function validateCivicImage(
  selectedCategory: string,
  photoInput: string
  ...
  const targetCategory =
    selectedCategory === 'WATER_PROBLEM' ? 'STORM_WATER_DRAIN' : selectedCategory;

  // Run Visual Semantic Analysis
  const analysis = analyzeVisualCharacteristics(buffer, mime, photoInput, targetCategory);

  let decision: 'MATCH' | 'MISMATCH' | 'UNCERTAIN' = 'UNCERTAIN';
  ...
  if (analysis.isUnclear || analysis.detectedCategory === 'UNKNOWN') {
    ...
    analysis.detectedCategory === targetCategory ||
